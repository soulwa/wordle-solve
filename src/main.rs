use std::{error::Error, io::{self, Error}};

static FIVELETTERS: &'static str =
    concat!("WHICH,THEIR,WOULD,THERE,COULD,ABOUT,OTHER,THESE,FIRST,AFTER,THINK,",
...
fn main() -> Result<(), Box<dyn Error>> {
	...
	loop {
		...
		Ok(n) => {
		    if n != 4 && n != 5 {
			println!("expected guess to be 4-5 letters");
		    } else {
			break Some(command_line.clone());
		    }
		}
		Err(_) => {
		    eprintln!("error reading from the command line");
		    return Err("IO error")?;
		}
	    }
	}
    {
	println!("what is the guess status?");
...
fn prompt_input<F>(prompt: &str, validator: F) -> Result<String, Error>
where F: Fn(Str) -> Result<(), <Box <dyn Error>>> {
